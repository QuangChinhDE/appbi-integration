...
#: How far the clocks of two API replicas may disagree before a token issued
#: by one is rejected by the other.
#:
#: Not a relaxation of `exp`: thirty seconds on a twelve-hour session is
#: nothing. It is about `iat` and `nbf`, which are compared against *now* with
#: no tolerance by default -- so a replica whose clock is a second behind the
#: one that issued a token answers "The token is not yet valid", and the user
#: sees "your session has expired" moments after signing in. NTP drift of a
#: second is ordinary; a control plane that cannot survive it is not one that
#: can be scaled.
