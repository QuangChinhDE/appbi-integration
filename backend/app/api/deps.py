...
async def current_user(
    ...
    appbi_workflow_session: Annotated[
        str | None, Cookie(alias=settings.session_cookie_name)
    ] = None,
) -> User:
    token = _bearer(authorization) or appbi_workflow_session
    if not token:
    ...
    # A token issued before the last password change is not a session any more.
    # Tokens minted before this field existed carry no `sv`, which reads as 0 --
    # the value every untouched account still has.
...
async def request_context(
    ...
    """Resolve the tenant from the session, or from a header naming a workspace
    the caller can reach. Never from a request body (guardrail 19).

    Also the chokepoint for a forced password change: an account created from an
    invite can sign in and change its password, and nothing else. Enforced here
    rather than per route, because "the one route somebody forgot" is how that
    control fails.
    """
    ...
    # cannot reach it, falling back to another workspace would answer a question
    # nobody asked -- and the caller, believing they addressed the workspace
    # they named, would write into a different tenant's data. Refuse instead.
    #
    ...
    # lock somebody out with no way to clear it but deleting a cookie they
    # cannot see.
    chosen = None
    if x_workspace_id:
        try:
            wanted = uuid.UUID(str(x_workspace_id))
        except (TypeError, ValueError):
            raise ForbiddenError("X-Workspace-Id không hợp lệ.") from None
        chosen = next((m for m in memberships if m.workspace_id == wanted), None)
        if chosen is None:
            raise ForbiddenError("Bạn không truy cập được workspace này.")
    elif claims.get("ws"):
        try:
            remembered = uuid.UUID(str(claims["ws"]))
            chosen = next((m for m in memberships if m.workspace_id == remembered), None)
        except (TypeError, ValueError):
            chosen = None
    chosen = chosen or memberships[0]
