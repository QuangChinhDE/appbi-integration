...
| **W08a** | **Fan-out per row.** Manual -> HTTP GET returning an array of 100 -> HTTP GET per item, with the item's own id in the URL. Asserts 100 requests, correctly paired, in order. **The defining App-to-App pattern — and it needs no new node** | — | 5.1, 7.2, 7.3 |
...
| **W08b** | **Fan-out from a nested payload.** HTTP GET returning `{data: [...]}` -> Split Out -> HTTP GET per item. The half of W08 that genuinely needs a new node: a nested array arrives as **one** item and does not fan out by itself (measured — `SPIKE_FANOUT_FINDINGS.md`) | `item_lists` | 5.3, 5.4 |
...
| **W12** | **Rate-limited loop.** Split In Batches (size 10) -> IF -> HTTP POST on the true branch only -> back to the loop. Termination when every item takes the false branch. **Deferred to last** — this is the only workflow that genuinely needs `split_in_batches`, and the node is the riskiest in the catalogue | `split_in_batches` | 5.2, 3.7, 4.10 |
...
| **W15** | **Resilience.** A long-running W08a with the engine killed mid-run: the execution becomes `ENGINE_INTERRUPTED`, the UI says runs are paused and stays readable, and after the engine returns a fresh run succeeds | `item_lists`, `split_in_batches` | **6.2, 6.3**, 6.1 |
...
- **W05, W10, W14 and W15 are the ones that matter most**, and they are the
  ...
  respectively.
- **W08a is the single most representative workflow in the set.** If it works,
  the product can do App-to-App integration; if it does not, the catalogue size
  is irrelevant. A spike has since shown its core passes at the engine level
  (100 items, 100 correctly paired requests, no new node) — W08a promotes that
  to a real workflow driven through the UI, which is a different claim.
- Tier 1 needs no new node at all. **Seven of the fifteen are writable today**,
...
- **The counts moved after the spike, and that is the point.** `split_in_batches`
  went from three workflows to one, `item_lists` from five to four. Both were
  guesses; one measurement corrected them before either node was certified.

## Coverage back to the catalogue

| Node | Reference workflows needing it |
|---|---|
| `item_lists` | W07, W08b, W09, W10 — **4** |
| `split_in_batches` | W12 — **1** |
