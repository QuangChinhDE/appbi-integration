 ...
 *
 * The id that ties a `<label>` to its control is generated here and shared
 * through context, so `Label` and `Input`/`Select`/`Textarea` wire themselves
 * up. Remembering to pass `htmlFor` and a matching `id` at every call site is
 * something people forget, and when they do the field becomes unreachable by
 * name -- to a screen reader, and to anything else that finds a control the way
 * a person does (SRS 40).
 */
...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(function Input(
  { className, leadingIcon, trailingIcon, size = 'md', invalid, id, ...props },
  ref,
) {
...
export const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(function Textarea(
  { className, invalid, rows = 3, id, ...props },
  ref,
) {
  const fieldId = React.useContext(FieldContext);
  return (
...
export const Select = React.forwardRef<HTMLSelectElement, SelectProps>(function Select(
  { className, size = 'md', invalid, children, id, ...props },
  ref,
) {
  const fieldId = React.useContext(FieldContext);
  return (
...
export function FieldError({
   ...
   *
   * Taken here rather than by a wrapper element: a caller that wrapped this in
   * its own `<p>` to carry the id produced `<p><p>`, which React reports as
   * invalid nesting and which browsers silently repair by closing the outer
   * paragraph -- leaving the described element outside the element the id is
   * on, so the association a screen reader follows points at nothing.
   */
  id?: string;
}) {
  if (!children) return null;
  return <p id={id} className="mt-1 text-tiny text-danger">{children}</p>;
}
