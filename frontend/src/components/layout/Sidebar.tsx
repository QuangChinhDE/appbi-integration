...
import {
  Bell, Blocks, Check, ChevronLeft, ChevronRight, ChevronsUpDown, Globe, Home,
  KeyRound, LogOut, PlayCircle, Radar, ScrollText, Settings, Workflow, X, Zap,
} from 'lucide-react';

import { authApi, opsApi, workflowApi } from '@/lib/api';
...
interface NavItem {
  ...
  module?: Module;
  action?: Action;
  /** Hidden until this workspace has a workflow. See `NavGroup.advanced`. */
...
interface NavGroup {
  ...
  items: NavItem[];
  /**
   * Folded away until the workspace has its first workflow.
   *
   * On a fresh deployment every module is equally prominent, so the two things
   * a new user actually needs — build something, then watch it run — sit beside
   * the node library, the audit log and alert rules. None of those mean
   * anything before a workflow exists.
   */
  advanced?: boolean;
}

// Grouped by user intent (SRS 6.1).
const NAV_GROUPS: NavGroup[] = [
  {
    items: [{
      labelKey: 'sidebar.overview', href: '/overview',
      icon: <Home className="h-4 w-4" />, module: 'monitoring',
    }],
  },
  {
    ...
    items: [
      {
        labelKey: 'sidebar.workflows', href: '/workflows',
        icon: <Workflow className="h-4 w-4" />, module: 'workflows',
      },
      {
        labelKey: 'sidebar.credentials', href: '/credentials',
        icon: <KeyRound className="h-4 w-4" />, module: 'credentials',
      },
    ],
  },
  {
    ...
    items: [
      {
        labelKey: 'sidebar.executions', href: '/executions',
        icon: <PlayCircle className="h-4 w-4" />, module: 'executions',
      },
      // Monitoring dashboards and alert rules describe a system that is already
      // running; before the first workflow they are empty screens.
      {
        labelKey: 'sidebar.monitoring', href: '/monitoring',
        icon: <Radar className="h-4 w-4" />, module: 'monitoring', advancedItem: true,
      },
      {
        labelKey: 'sidebar.alerts', href: '/alerts',
        icon: <Bell className="h-4 w-4" />, module: 'alerts', advancedItem: true,
      },
    ],
  },
  {
    ...
    items: [
      {
        labelKey: 'sidebar.nodes', href: '/nodes',
        icon: <Blocks className="h-4 w-4" />, module: 'nodes',
      },
      {
        labelKey: 'sidebar.audit', href: '/audit',
        icon: <ScrollText className="h-4 w-4" />, module: 'audit',
      },
    ],
  },
];
...
export function Sidebar({
  ...
  // The badge polls from every screen, so a role without `alerts` would produce
  // a 403 on every page — noise in the logs and a request that can never
  // succeed being retried forever.
  ...
  // First run: no workflow has ever been created here. Until one has, the
  // sidebar shows the path through the product rather than every module at
  // once. Once a workflow exists the whole navigation returns permanently.
  const probe = useQuery({
    queryKey: qk.workflows(workspaceId, { probe: 'first-run' }),
    queryFn: () => workflowApi.list({ limit: 1 }),
    enabled: Boolean(user),
    staleTime: 60_000,
  });
  const firstRun = probe.data?.items?.length === 0;
  ...
  return (
    <>
      ...
      >
          ...
          <div className="relative px-2 pb-2">
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                {/* The stored value is an enum. AUTOMATION_BUILDER under a
        ...
        {/* Nav */}
        <nav className={cn('flex-1 overflow-y-auto px-2 py-1',
          collapsed ? 'space-y-1.5' : 'space-y-3')}>
          {visibleGroups.map((group, groupIndex) => (
            <div key={group.labelKey ?? `group-${groupIndex}`}>
              {group.labelKey && (collapsed ? (
                groupIndex > 0 && (
                  <div
                    className="mx-auto mb-1.5 h-px w-6 bg-[rgb(var(--border-line))]"
                    aria-hidden
                  />
                )
              ) : (
                <p className="px-2.5 pb-1 pt-0.5 text-tiny font-emphasis uppercase tracking-[0.1em] text-text-tertiary">
                  {t(group.labelKey)}
                </p>
              ))}
                  ...
                  return (
                    <li key={item.href}>
                      ...
                      >
                        ...
                        {!collapsed && (
                          <>
                            <span className="flex-1 truncate text-caption font-emphasis">
                              {label}
                            </span>
                            {badge && (
                              <span className="rounded-full bg-danger px-1.5 text-tiny font-strong text-white">
                                {badge > 99 ? '99+' : badge}
                              </span>
                            )}
                          </>
        ...
        <div className="border-t border-[rgb(var(--border-line))]">
          {user && (
            <div ref={menuRef} className="relative px-2 pt-2">
              <button
                type="button"
                // A disclosure that announces neither its state nor that it
                // opens a menu is invisible to assistive tech.
                aria-haspopup="menu"
              ...
              {menuOpen && (
                ...
                >
                  ...
                  {hasPermission(permissions, 'settings', 'view') && (
                    <>
                      <MenuLink
                        href="/settings/workspace" onClick={() => setMenuOpen(false)}
                        icon={<Settings className="h-3.5 w-3.5" />}
                        label={t('settings.workspace')}
                      />
                      <MenuLink
                        href="/settings/access" onClick={() => setMenuOpen(false)}
                        icon={<Settings className="h-3.5 w-3.5" />}
                        label={t('settings.access')}
                      />
                      <MenuLink
                        href="/settings/engine" onClick={() => setMenuOpen(false)}
                        icon={<Settings className="h-3.5 w-3.5" />}
                    ...
                    <div className="inline-flex overflow-hidden rounded-md border border-[rgb(var(--border-line))]">
                      {(['vi', 'en'] as const).map((code) => (
                        <button
                          key={code}
                          type="button"
                          // The active locale is signalled by background alone;
                          // without aria-pressed a screen reader cannot tell
                          // which one is on.
