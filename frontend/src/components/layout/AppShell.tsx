...
const COLLAPSE_KEY = 'appbi.workflow.sidebar-collapsed';

/**
 * Engine degradation is a banner, not a blocked app: the product must stay
 * readable from its own database when the engine is down (SRS 9.6).
 */
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    ...
    // The account authenticated but cannot use the product yet. Without this
    ...
    // deployment did to its own bootstrap admin.
    if (error.status === 403 && error.code === 'PASSWORD_CHANGE_REQUIRED') {
      router.replace('/change-password');
    }
  }, [error, router]);

  // Belt and braces: `/auth/me` answers for an account that must change its
  // password rather than failing, so there is no error to react to on a reload.
  React.useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-surface-0">
      ...
      {/* A viewport-tall flex column: whatever bars sit above `main` -- the
          mobile header, the health banner -- reduce its share rather than
          adding their height on top of it and pushing the page past the fold. */}
      <div className={cn('flex h-screen flex-col transition-[padding] duration-300',
        ...
        <div className="flex h-12 items-center gap-2 border-b border-[rgb(var(--border-line))] bg-surface-1 px-4 lg:hidden">
          ...
          <span className="text-caption font-strong text-text-primary">{t('app.name')}</span>
        </div>
        <EngineHealthBanner />
        {/* A flex column, not a block: the editor fills the viewport and sizes
            its canvas and panels against this. */}
