...
from modules.connectors.apps.base_table.common.constants import API_PREFIX


def normalize_table_domain(domain: str) -> str:
    ...
    if cleaned.startswith("table."):
        cleaned = cleaned[len("table."):]
    if not cleaned:
        raise ValueError("domain is invalid")
    return cleaned


class TableCredentials(BaseModel):
    ...
    def validate_domain(cls, value: str) -> str:
        return normalize_table_domain(value)
    ...
    def base_url(self) -> str:
        return f"https://table.{self.domain}{API_PREFIX}"
