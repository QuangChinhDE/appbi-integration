...
from modules.connectors.apps.base_timeoff.common.constants import API_PREFIX


def normalize_timeoff_domain(domain: str) -> str:
    ...
    if cleaned.startswith("timeoff."):
        cleaned = cleaned[len("timeoff."):]
    if not cleaned:
        raise ValueError("domain is invalid")
    return cleaned


class TimeoffCredentials(BaseModel):
    ...
    def validate_domain(cls, value: str) -> str:
        return normalize_timeoff_domain(value)
    ...
    def base_url(self) -> str:
        return f"https://timeoff.{self.domain}{API_PREFIX}"
