...
from modules.connectors.apps.base_hrm.common.constants import API_PREFIX


def normalize_hrm_domain(domain: str) -> str:
    ...
    if cleaned.startswith("hrm."):
        cleaned = cleaned[len("hrm."):]
    if not cleaned:
        raise ValueError("domain is invalid")
    return cleaned


class HrmCredentials(BaseModel):
    ...
    def validate_domain(cls, value: str) -> str:
        return normalize_hrm_domain(value)
    ...
    def base_url(self) -> str:
        return f"https://hrm.{self.domain}{API_PREFIX}"
