...
from modules.connectors.apps.base_goal.common.constants import API_PREFIX


def normalize_goal_domain(domain: str) -> str:
    ...
    if cleaned.startswith("goal."):
        cleaned = cleaned[len("goal."):]
    if not cleaned:
        raise ValueError("domain is invalid")
    return cleaned


class GoalCredentials(BaseModel):
    ...
    def validate_domain(cls, value: str) -> str:
        return normalize_goal_domain(value)
    ...
    def base_url(self) -> str:
        return f"https://goal.{self.domain}{API_PREFIX}"
