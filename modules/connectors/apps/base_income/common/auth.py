...
from modules.connectors.apps.base_income.common.constants import API_PREFIX


def normalize_income_domain(domain: str) -> str:
    ...
    if cleaned.startswith("income."):
        cleaned = cleaned[len("income."):]
    if not cleaned:
        raise ValueError("domain is invalid")
    return cleaned


class IncomeCredentials(BaseModel):
    ...
    def validate_domain(cls, value: str) -> str:
        return normalize_income_domain(value)
    ...
    def base_url(self) -> str:
        return f"https://income.{self.domain}{API_PREFIX}"
