...
const FolderPickerModal = ({ wizard }) => {
  ...
  return (
    ...
    <AppModalShell
      ...
      footer={
        <>
          <button onClick={() => setGoogleFolderModal(false)} className="rounded-md border border-[rgb(var(--border-strong))] px-4 py-2 text-caption font-emphasis text-text-secondary transition-colors hover:bg-surface-2">Cancel</button>
          <button onClick={handleSelectCurrentFolder} className="flex items-center gap-2 rounded-md bg-brand px-4 py-2 text-caption font-strong text-white transition-colors hover:bg-brand-hover">
    ...
    >
      {loadingDrives ? <SpinCenter text="Loading drives…" /> : (
        <div className="space-y-3">
          ...
          {/* Breadcrumb */}
          <div className="bg-surface-2 rounded-md px-3 py-2 flex flex-wrap gap-1 items-center text-tiny">
          ...
          {isServiceAccountDestinationAuth && (
            <div className="space-y-2">
              <div className="border-t border-[rgb(var(--border-line))] pt-2">
                <p className="text-tiny font-emphasis text-text-tertiary mb-2">Shared to this service account</p>
                ...
                <div className="flex gap-2 mb-2">
                  <input value={sharedFolderReference} onChange={e => setSharedFolderReference(e.target.value)}
                    ...
                    className="flex-1 rounded-md border border-[rgb(var(--border-strong))] bg-surface-0 px-3 py-1.5 text-caption text-text-primary placeholder:text-text-quaternary focus:border-brand focus:shadow-focus-brand focus:outline-none transition-colors" />
                  <button onClick={handleResolveSharedFolder} disabled={resolvingSharedFolder}
                    className="px-3 py-1.5 text-caption bg-brand text-white rounded-md hover:bg-brand-hover disabled:opacity-50 flex items-center gap-1">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => loadSharedFolders(sharedFolderQuery.trim())} disabled={loadingSharedFolders}
                    className="px-3 py-1.5 text-caption border border-[rgb(var(--border-strong))] rounded-md hover:bg-surface-2 disabled:opacity-50">Search</button>
                </div>
              </div>
                ...
                {loadingSharedFolders ? <SpinCenter text="Loading…" /> : sharedFolders.length === 0 ? <Empty description="No directly shared folders found" /> : sharedFolders.map(folder => (
                  <div key={folder.id} className="flex items-center gap-3 px-3 py-2.5 border-b border-[rgb(var(--border-line))] last:border-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-tiny text-text-tertiary">Drive: {resolveDriveName(folder.drive_id, folder.drive_name || null)}</div>
                    ...
                    <div className="flex gap-1 shrink-0">
                      <button onClick={() => openFolderLocation(folder)} className="px-2 py-1 text-tiny border border-[rgb(var(--border-strong))] rounded hover:bg-surface-2">Open</button>
                      {folder.drive_id
                        ? <button onClick={() => applyGoogleFolderSelection(folder)} className="px-2 py-1 text-tiny bg-brand text-white rounded hover:bg-brand-hover">Use</button>
                        : <button disabled title="Cannot use My Drive folder with service account" className="px-2 py-1 text-tiny bg-brand/20 text-white rounded opacity-50 cursor-not-allowed">Use</button>}
                    </div>
                  </div>
                ))}
              </div>
